...
SELECT FLOOR(34.567/10)*10 FROM DUAL; -- 일의자리에서 내림
...
SELECT INITCAP('welcome to oracle') FROM DUAL; -- 어절 단위로 나눠서 첫 문자만 대문자
...
SELECT CONCAT(CONCAT ('AB','CD'),CONCAT('EF','GH')) FROM DUAL;-- 불편함
    -- "SMITH는 MANAGER다" 포맷으로EMP 테이블 데이터를 출력
...
-- SUBSTRB(STR, 시작바이트위치, 바이트갯수) : STR을 시작부터 BYTE갯수만큼 문자 추출
        -- O R A C L E
        -- 1 2 3 4 5 6
        ---6-5-7-3-2-1
...
SELECT SUBSTRB ('데이터베이스', 4, 3) FROM DUAL;
    ...
    SELECT *FROM EMP WHERE SUBSTR(HIREDATE, 4, 2) = '09';-- DATE 형도 가능
    DESC EMP;
    -- 9일에 입사한 사원의 모든 필드 
...
-- (5) LPAD(STR, 자리수, 채울문자) : STR을 자리수만큼 확보하고 왼쪽 빈자리에 채울 문자로 출력
...
SELECT LPAD('ORACLE',10) FROM DUAL;
    ...
    -- EX. ENAME : 최장 10글자, SAL : 5자리.2자리
    SELECT LPAD(ENAME, 10, '-'), LPAD(SAL, 5,'$') FROM EMP; -- 문자는 왼쪽
    ...
    SELECT EMPNO,
            INITCAP(ENAME) ENAME, LPAD(LPAD(SUBSTR(JOB,-1,1),LENGTH(JOB),'*'),9) JOB,
            SUBSTR(HIREDATE,1,6) || '**'
        FROM EMP;
...
-- (6) INSTR(STR, 찾을문자) : STR에서 찾을 문자가 첫번째로 나오는 위치(처음1, 없으면 0)    
...
-- (2) 날짜계산 : 오라클 기본타임(문자, 숫자, 날짜) 중 숫자, 날짜 타입은 연산가능
SELECT 
        TO_CHAR(SYSDATE, 'RR/MM/DD HH:MI'),
        TO_CHAR(SYSDATE+1, 'RR/MM/DD HH:MI')
    FROM EMP;
    
    -- EX. 개강시점(24.08.26 09:30)부터 현재까지 몇일 지났나
    SELECT
...
-- (4) NEXT_DAY(특정시점, '토') : 특정시점부터 처음돌아오는 토요일
SELECT NEXT_DAY(SYSDATE,'토') FROM DUAL;

-- (5) ADD_ MONTHS(특정시점, 개월수) : 특정시점부터 몇개월 후 
SELECT ADD_MONTHS(SYSDATE,1) FROM DUAL;
    ...
    SELECT * FROM EMP;
    ROLL BACK; -- DML언어(데이터입력, 수정, 삭제) 취소
...
SELECT TRUNC (SYSDATE, 'YEAR') FROM DUAL;--올해 1월1일
    ...
    SELECT ENAME, HIREDATE, ROUND(HIREDATE+1, 'MONTH') + 14일 월급날 FROM EMP;
...
--(1) TO_CHAR(날짜형,'출력형식')
    -- YYYY 년도 4자리/ RR 년도 2자리/MM 월 / DD 일/ DY 요일/ DAY N요일
    -- HH24/ HH12/ HH(12시간) / MI 분/ SS초/ AM 오전/오후
    -- 출력 형식에 문자를 포함할 경우 ""
...
SELECT TO_CHAR(1234.56 '9,999.9') FROM DUAL; --소수점자리가 부족할 경우 반올림
...
-- 5. NVL(널일수도 있는 데이터, 널이면 대신 할 값) - 매개변수 2개의 타입 일치
    ...
    -- EX. 사원 이름, 직속상사의 사번(직속상사가 업으면 CEO로 출력)
...
SELECT HIREDATE, TO_CHAR(HIREDATE, 'MM') FROM EMP;--(문자로 추출)
SELECT HIREDATE, EXTRACT(DAY FROM HIREDATE) FROM EMP;--(숫자로 추출)
SELECT HIREDATE, TO_CHAR(HIREDATE, 'DD') FROM EMP;--(문자로 추출)
...
SELECT ENAME, HIREDATE, TRUNC(MONTHS_BETWEEN(SYSDATE,HIREDATE)),
            SAL, SAL*TRUNC(MONTHS_BETWEEN(SYSDATE,HIREDATE)) TOTSAL 
        FROM EMP;
...
SELECT HIREDATE, EMPNO, ENAME FROM EMP WHERE TO_CHAR(HIREDATE,'MM')='12';
SELECT HIREDATE, EMPNO, ENAME FROM EMP WHERE EXTRACT(MONTH FROM HIREDATE)='12';
...
--13. 사원 테이블에서 부서번호가 20인 사원의 사번, 이름, 직무, 급여를 출력하시오(급여는 앞에 $를 삽입하고 3자리마
--다 ,를 출력한다
