...
SELECT * FROM EMP WHERE ENAME = 'SMITH'; -- 14행
SELECT * FROM DEPT; -- 4행
SELECT * FROM EMP, DEPT WHERE ENAME = 'SMITH'; -- CROSS JOIN : 4행(1*4)

-- ★ 1.EQUI JOIN(동등 JOIN = 공통필드값이 일치되는 조건만 JOIN)
...
SELECT *FROM EMP E, DEPT D
    ...
    -- EX. 82년도에 입사한 10번,20번의 부서 직원의 이름, 급여 근무지 출력(급여순)
    ...
    -- 탄탄EX. COMM이 NULL이고 SAL이 1200이상인 사원의 이름, 급여,부서번호, 부서명
                        --(부서명 순, 같으면 급여 큰 순으로 정렬)
...
AND SAL>=LOSAL AND SAL<=HISAL;
    ...
    -- EX. 모든 사원의 사번, 이름, 상사사번, 급여, 급여등급(1등급,2등급...)1
    ...
    -- 탄탄EX2. 이름, 급여, 입사일, 급여등급(신입사원 순으로 출력))
    ...
    -- EX5. 이름, 급여, 등급, 입사일, 근무지(81년 입사한 직원만 등급 큰순)
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME
    ...
    SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME
        FROM EMP W, EMP M
        WHERE W.MGR=M.EMPNO; -- 13행(KING의 MGR이 NULL이기 때문에 EMPNO에 NULL)
    -- EX. "SMITHD의 상사는 FORD" 출력
    ...
    --탄탄EX2. SCOTT과 동일한 부서번호인 사원의 이름을 출력
    ...
    SELECT E1.ENAME, E1.DEPTNO, E2.ENAME, E2.DEPTNO
        FROM EMP E1, EMP E2
        WHERE E1.ENAME='SCOTT'AND E1.DEPTNO=E2.DEPTNO AND E2.ENAME!='SCOTT';
    --탄탄EX3. SCOTT과 동일한 지역에서 근무하는 사원의 이름을 출력
...
--5. 상사가 없는 직원과 상사가 있는 직원 모두에 대해 이름, 급여, 부서코드, 
    --부서명, 근무지, 직속상사명을 출력하시오(
...
--8. 이름, 급여, 등급, 부서명, 연봉, 직속상사명. 연봉=(급여+comm)*12으로 계산
    SELECT W.ENAME, W.SAL,DNAME,GRADE,(W.SAL+NVL(W.COMM,0))*12 ANNUALSAL, M.ENAME MANAGER
        FROM EMP W, EMP M, DEPT D, SALGRADE 
        WHERE W.SAL BETWEEN LOSAL AND HISAL  
            AND W.MGR=M.EMPNO
            AND W.DEPTNO=D.DEPTNO;
...
--11. 사원명, 상사명, 상사의 상사명을 검색하시오(self join)
    SELECT W.ENAME, M.ENAME MANAGER, HM.ENAME HEADMANAGER
        FROM EMP W, EMP M, EMP HM
        WHERE W.MGR=M.EMPNO AND M.MGR=HM.EMPNO;
