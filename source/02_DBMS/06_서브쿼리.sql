...
-- (1) ALL : 서브쿼리 결과가 모두 만족하면 참
    ...
    SELECT * 
        FROM EMP 
            WHERE SAL> ALL(SELECT SAL FROM EMP WHERE DEPTNO='30');-- 메인쿼리
    
    -- (== 30번 부서 최대 급여보다 큰 직원의 모든 필드)
    SELECT * FROM EMP WHERE SAL>(SELECT MAX(SAL) FROM EMP WHERE DEPTNO='30');
    
-- (2) ANY(=SOME) : 서브쿼리 결과가 하나라도 만족하면 참
    --EX. 30번 부서 직원 중 한명의 급여보다 급여가 큰 직원의 모든 필드
    SELECT SAL FROM EMP WHERE DEPTNO=30; -- SUBQ
    SELECT * FROM EMP WHERE SAL> ANY(SELECT SAL FROM EMP WHERE DEPTNO=30);
    -- (==) 30번 부서 최소급여보다 큰 직원의 모든 필드
    SELECT * FROM EMP WHERE SAL> (SELECT MIN(SAL) FROM EMP WHERE DEPTNO=30);

-- (3) IN : 
    -- EX. 부서별로 입사일이 가장 늦은 사람의 모든 이름, 입사일, 부서번호
    SELECT DEPTNO, MAX(HIREDATE) FROM EMP GROUP BY DEPTNO;--다중행 다중열 SUBQ
...
-- (4) EXISTS : 서브쿼리 결과가 존재하면 참
    ...
    SELECT EMPNO, ENAME, SAL FROM EMP MANAGER
                --읽어드린 EMP데이터의 EMPNO가 MGR에 존재하지 않는지?
        WHERE NOT EXISTS (SELECT * FROM EMP WHERE MANAGER.EMPNO=MGR);
    
    SELECT M.EMPNO, M.ENAME, M.SAL
        FROM EMP W, EMP M
        WHERE W.MGR(+)=M.EMPNO AND W.ENAME IS NULL; --SELF JOIN이용
...
SELECT E.*, DNAME, LOC, GRADE
    ...
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
        AND (E.DEPTNO, SAL) IN (SELECT E.DEPTNO, MAX(SAL)
                            FROM EMP GROUP BY DEPTNO)
    ORDER BY E.DEPTNO;
...
-- 탄탄4. 응용심화 : 입사일 분기별로 가장 높은 급여를 받는 사람들의 분기, 사번, 이름, JOB, 상사사번, 입사일, 급여, 상여를 출력하세요
    ...
    SELECT TO_CHAR(HIREDATE, 'Q') ,MAX(SAL)
        FROM EMP GROUP BY TO_CHAR(HIREDATE,'Q'); --서브 쿼리
...
-- 탄탄6. SALESMAN 모든 사원들 보다 급여를 많이 받는 사원들의 이름과 급여와 직급(담당 업무)를 출력하되 영업 사원은 출력하지 않는다.(ALL이용)
    ...
    -- = JOB이 SALESMAN인 사원들 최대 급여보다 많이 받는 사원 정도
    SELECT ENAME, SAL, JOB
        FROM EMP
            WHERE SAL> (SELECT MAX(SAL) FROM EMP WHERE JOB = 'SALESMAN'); --단일행 서브쿼리
...
-- 17. SALES부서 사원의 이름, 업무
    SELECT ENAME, JOB
        FROM EMP
            WHERE DEPTNO = (SELECT DEPTNO  FROM DEPT WHERE UPPER(DNAME) LIKE '%SALES%');
...
-- 24. 업무별로 평균 월급보다 적은 월급을 받는 사원을 부서번호, 이름, 급여, JOB, JOB평균
    SELECT DEPTNO, ENAME, SAL --, JOB, ROUND((SELECT AVG(SAL) FROM EMP WHERE E.JOB=JOB)) AVGSAL
        FROM EMP E
...
-- 26.  말단 사원의 사번, 이름, 업무, 부서번호
    ...
    -- WHERE EMPNO !=7839 AND EMPNO != NULL AND....        