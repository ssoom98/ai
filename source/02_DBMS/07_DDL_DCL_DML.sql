...
--       DML(SELECT, INSERT, UPGRAD, DELETE) +
...
CREATE TABLE BOOK(
    ...
    PUBLISHER VARCHAR2(30), --PUBLISHER 필드의 타입은 문자 30 byte.
...
CREATE TABLE BOOK(
    ...
    -- DEPT 01테이블: DEPTNO(숫자2:PK), DNAME(문자 14), LOC(문자 13)
CREATE TABLE DEPT01(
    DEPTNO NUMBER(2),
    DNAME VARCHAR(14),
    LOC VARCHAR(13),
    ...
    --EX. EMP와 유사한 EMP01 테이블 : EMPNO(숫자4-PK), ENAME(문자10), SAL(숫자7,2), 
...
--외래키로 연결할 경우 :참조테이블(DEPTN01)에 데이터를 먼저 INSERT
INSERT INTO DEPT01 VALUES (10,'신림','IT');
INSERT INTO EMP01 VALUES (1000,'홍', 9000,10);
DROP TABLE DEPT01;--참조하는 테이블이 존재 할경우 테이블 삭제가 불가능
COMMIT;

-- 서브 쿼리를 이용한 테이블 생성
CREATE TABLE EMP02
    AS
    SELECT * FROM EMP; -- 서브쿼리 결과로 EMP02테이블 생성 후 , 데이터 들어감 (제약조건 미포함)
