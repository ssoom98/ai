--[VII] DDL, DCL, DML
-- SQL = DDL(테이블 생성, 테이블 삭제, 테이블 구조 변경, 테이블 모든 데이터 제거, 테이블명 변경) +
...
CREATE TABLE EMP03 --EMP의 특정필드
    AS
    SELECT EMPNO, ENAME, DEPTNO FROM EMP;
SELECT *FROM EMP03;

CREATE TABLE EMP04 --EMP의 특정 행
    AS
...
ALTER TABLE EMP03 ADD(JOB VARCHAR2(20), SAL NUMBER(7,2)); --추가된 필드의 데이터는 NULL로 채워짐
ALTER TABLE EMP03 ADD COMM NUMBER(7,2);
SELECT * FROM EMP03;

-- (2)필드 수정(MODIFY)
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(4); -- 숫자 데이터가 들어있으면 숫자로만 변경
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(5);
ALTER TABLE EMP03 MODIFY EMPNO NUMBER(4); -- 숫자는 줄이는게 불가
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(100);
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(10); -- 문자데이터필드는 늘이거나 줄이는거 가능(데이터보다 줄이는건 불가)
SELECT MAX(LENGTH(ENAME)) FROM EMP;
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(6);--가능
ALTER TABLE EMP03 MODIFY EMPNO VARCHAR2(5);--불가능
...
ALTER TABLE EMP03 DROP COLUMN ENAME;-- 데이터까지 삭제 (취소불가)
SELECT *FROM EMP03;
...
SELECT * FROM EMP02;
TRUNCATE TABLE EMP02; -- ROLLBACK불가
SELECT * FROM EMP02;
...
-- 데이터 딕셔너리 뷰의 종류
    --(1)USER_xxx : 현계정이 소유하고 있는 객체(테이블,제약조건, 뷰, 인덱스)
        --USER_TABLE, USER_CONSTRAINTS, USER_VIEWS, USER_INDEXES
        SELECT * FROM USER_TABLES;
        SELECT * FROM USER_CONSTRAINTS;
        SELECT * FROM USER_VIEWS;
        SELECT * FROM USER_INDEXES;
    -- (2)ALL_xxx : 현 계정에서 접근 가능한 객체(테이블, 제약조건, 뷰, 인덱스)
        -- ALL_TABLES, ALL_ CONSTRAINTS, ALL_VIEWS, ALL_INDEXES
...
--1. INSERT INTO 테이블명 VALUES(값1, 값2, ...); 모든 필드 데이터 입력
    -- INSERT INTO 테이블명 (필드명1, 필드명2, ..) VALUES(값1, 값2,..); 언급된 필드외는 NULL로
SELECT * FROM DEPT01;
...
INSERT INTO DEPT01 (DEPTNO, DNAME) VALUES (90, 'OPERATION');-- 묵시적으로 NULL추가
SELECT * FROM DEPT01;

-- 서브쿼리를 이용한 INSERT
INSERT INTO DEPT01 SELECT * FROM DEPT WHERE DEPTNO<40;
SELECT * FROM DEPT01;
    --EX. BOOK(ID:11, 책이름은 스포츠의 의학, 출판사는 한솔출판, 출판일 오늘, 가격은 90,000)
    SELECT * FROM BOOK;
    INSERT INTO BOOK VALUES(11, '스포츠의 의학', '한솔출판', SYSDATE, TO_NUMBER('90000','99,999'));

    --EX. BOOK(ID:12, 책이름은 스포츠과학, 출판사는 NULL, 출판일 오늘, 가격은 50,000)
...
CREATE TABLE EMP01 AS SELECT EMPNO, ENAME, HIREDATE, SAL, DEPTNO FROM EMP;
    ...
    -- EX. 10번부서의 입사일을 오늘로, 부서번호를 30번으로 수정
    UPDATE EMP01 
    ...
    -- EX. EMP에서 SCOTT의 부서 번호는 30으로 
    --     JOB은 MANAGER, SAL과 COMM은 500$씩 인상, 상사를 'KING' 으로 수정
    COMMIT;
    ...
    UPDATE EMP01 
        SET (HIREDATE,SAL)=(SELECT HIREDATE,SAL FROM EMP01 WHERE ENAME='KING');
            
    SELECT * FROM EMP01;
    
-- 3. DELETE FROM 테이블명 [WHERE 조건]; --ROLLBACK 가능
SELECT * FROM EMP01;
DELETE FROM EMP01; --DML명령어는 취소가능 (DDL은 취소 불가)
ROLLBACK;
SELECT * FROM DEPT01;
...
INSERT INTO DEPT01 VALUES (10,'신림','IT');
INSERT INTO EMP01 VALUES (1000,'홍', 9000,10);
DELETE FROM DEPT01;-- 참조데이터가 있을 경우 에러
    SELECT * FROM EMP01;
...
UPDATE EMP01
    SET (DEPTNO,JOB)=(SELECT DEPTNO,JOB FROM EMP01 WHERE DEPTNO='20'AND JOB='MANAGER')
    WHERE ENAME='SCOTT';
    ROLLBACK;
-- 6. 부서명이 SALES인 사원을 모두 삭제하는 SQL작성
DELETE FROM EMP01 WHERE JOB LIKE '%SALES%';
...
INSERT INTO EMP1 (EMPNO,ENAME, JOB, DEPTNO)
    VALUES (1001,'HONG','MANAGER',10);--에러(10번부서 참조오류)

INSERT INTO DEPT1 SELECT * FROM DEPT;
SELECT* FROM DEPT;
INSERT INTO DEPT1 (DEPTNO, LOC, DNAME) VALUES(40,'신림', 'IT'); -- PRIMARY KEY 제약조건오류
INSERT INTO DEPT1 VALUES (50, 'SALES', '신림'); -- UNIQUE 오류
INSERT INTO DEPT1 VALUES (50, 'IT', NULL); -- NOT NULL 제약조건
INSERT INTO DEPT1 (DEPTNO, DNAME) VALUES (50,'IT');-- NOTNULL
SELECT* FROM EMP1;

INSERT INTO EMP1 (EMPNO,ENAME, JOB, DEPTNO)
    VALUES(1001,'HONG','MANAGER', 10); -- HIREDATE에는 DEFAULT 값 입력
INSERT INTO EMP1 (EMPNO, JOB, DEPTNO) VALUES (1002,'MANAGER',10);-- NOTNULL오류
...
INSERT INTO EMP1 (EMPNO, ENAME, JOB, HIREDATE, SAL, DEPTNO)
    VALUES (1003, 'PARK', 'MANAGER', TO_DATE('24/01/01', 'RR/MM/DD'), 1000, 20);
...
INSERT INTO BOOK (cCODE,bookNO,bookNAME, PUBLISHER)
    VALUES (100,'100A01','철학자의 삶', 'MBC출판');
INSERT INTO BOOK VALUES (400, '400a01', '이것이 DB다', '다음출판', 2022);

SELECT BOOKNO, BOOKNAME, PUBLISHER, PUBYEAR, CNAME, C.OFFICE_LOC
